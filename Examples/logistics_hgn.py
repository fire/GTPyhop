...
gtpyhop.declare_actions(
    drive_truck, load_truck, unload_truck, fly_plane, load_plane, unload_plane
)
...
def m_load_plane(state, o, plane):
    if (
        o in state.packages
        and plane in state.airplanes
        and state.at[o] == state.plane_at[plane]
    ):
...
gtpyhop.declare_unigoal_methods(
    "at", m_load_truck, m_unload_truck, m_load_plane, m_unload_plane
)
...
gtpyhop.declare_unigoal_methods(
    "at", move_within_city, move_between_airports, move_between_city
)
...
def main(do_pauses=True):
    ...
    gtpyhop.find_plan(
        state1, [("at", "package1", "location2"), ("at", "package2", "location3")]
    )
